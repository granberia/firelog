...
- [x] 엑스페리온 설정 봇 만들기
  ...
  - [x] 엑스페리온 세계관을 주입한 설정 봇 개발
    - TRPG 마스터링을 하기 위해 엑스페리온 설정을 정리하는 것 외에도, 엑스페리온 기반 캐릭터 AI를 만들기 위해 세계관 정보를 정리해야 한다.
    - 엑스페리온 장편 세션의 로그를 통해 캐릭터 AI를 만들기 위한 기반 설정들을 모두 지식 베이스로 만들었다. 이걸 그대로 넣으면 세계관 설정 봇으로 사용할 수 있다.
- [x] TRPG 세션 로그로부터 LLM을 위한 지식 베이스 추출 자동화
    - [x] 로그저장소 추출기 개발
      - 로그저장소의 내용을 프롬프트에 넣기 위한 txt 파일로 다운로드하는 유틸리티를 만들어야 한다. 로그저장소에서 특정 장편 세션 탭을 선택하고 그 세션 안의 모든 로그를 다운로드하도록 만들자.
    - [x] 지식 베이스 자동 업데이트 툴 개발
      - 추출기에서 가져온 txt 파일을 읽어서 Playwright로 구동한 구글 AI 스튜디오와 상호작용하며 지식 베이스를 자동으로 생성하고 관리하는 유틸리티를 개발하자.
      - 가능하면 API를 쓰고 싶은데 최신 모델인 제미니 3은 무료로 사용하려면 AI 스튜디오에서 사용해야 한다. 방법이 좀 고민된다.
- [x] TRPG 세션 지식 베이스 기반 세션 위키 페이지 개발
  - [x] 지식 베이스 파일들을 위키처럼 표시하는 페이지를 만들어야 한다. 추출된 지식 베이스 파일들이 존재하는 repo에 먼저 개발하고 배포한 뒤, 나중에 필요하다면 github API로 파일 목록을 가져오는 방식으로 바꾸는 별도 프로젝트로 나누자.
  - [x] 지식 베이스 자동 갱신 이후 검수를 편리하게 할 수 있도록 git diff처럼 changelog를 볼 수 있는 페이지도 위키에 만들어야 한다.
  - [x] 완성된 위키 페이지에 실제 세션 내용을 채워야 한다.
    - [x] 엑스페리온: 다고시안 듀얼단
    - [x] 엑스페리온: 페리아의 문장 1팀
- [x] TRPG 위키에 캐릭터 프롬프트를 제작하는 페이지 개발
  - [x] 세션 로그로부터 캐릭터 대사를 추출하는 페이지 개발
  - [x] 필요한 캐릭터 프롬프트 파일들을 합치는 페이지 개발
...
- [x] 파이널 판타지 14 설치 후 뭐라도 생각해보기
  ...
  - 게임 입문 당시에는 PvP를 열심히 즐겨보면 재미있지 않을까 생각했는데 팀 운에 좌우되는 부분들이 많고, PvP 게임은 스타크래프트가 더 재밌다. 길드원끼리 모여서 무언가 다같이 놀만한 것이 없었다.
- [ ] 페르소나 5 클리어
